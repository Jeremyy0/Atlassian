...
bot.on('message', function (user, userID, channelID, message, evt) {
  ...
  if(cmd === "find") {
    con.query("select tiles from (SELECT resource, GROUP_CONCAT(tile separator ', ') as tiles from locations group by resource) as T where resource=\""+[args[0].toLowerCase()]+"\"", function (err, result, fields) {
